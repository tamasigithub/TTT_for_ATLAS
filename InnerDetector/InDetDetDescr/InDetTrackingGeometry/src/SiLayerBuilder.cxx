...
const std::vector< const Trk::CylinderLayer* >* InDet::SiLayerBuilder::cylindricalLayers() const
{
  ...
  for (; sidetIter != m_siMgr->getDetectorElementEnd(); sidetIter++){
     ...
     if ((*sidetIter) && (*sidetIter)->isBarrel()){
       ...
       const InDetDD::SiDetectorElement* otherSide = (currentlayer<4) ? (*sidetIter)->otherSide() : nullptr;
       ...
       double currentRmax = (currentlayer<4) ? (*sidetIter)->rMax() : std::max(currentR,(*sidetIter)->rMax());
       double currentRmin = (currentlayer<4) ? (*sidetIter)->rMin() : std::min(currentR,(*sidetIter)->rMin());
