...
#cmt_local_tagfile = $(PixelCabling_tag).make
cmt_local_tagfile = $(bin)$(PixelCabling_tag).make
...
PixelCabling_extratags = -tag_add=target_PixelCabling
...
not_PixelCabling_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCabling_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCablingdirs :
	@if test ! -d $(bin)PixelCabling; then $(mkdir) -p $(bin)PixelCabling; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCabling
else
PixelCablingdirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCabling_makefile) : $(PixelCabling_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCabling.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCabling_extratags) build constituent_config -out=$(cmt_local_PixelCabling_makefile) PixelCabling
else
$(cmt_local_PixelCabling_makefile) : $(PixelCabling_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCabling) ] || \
	  [ ! -f $(cmt_final_setup_PixelCabling) ] || \
	  $(not_PixelCabling_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCabling.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCabling_extratags) build constituent_config -out=$(cmt_local_PixelCabling_makefile) PixelCabling; \
	  fi
endif
...
$(cmt_local_PixelCabling_makefile) : $(PixelCabling_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCabling.make"; \
	  $(cmtexe) -f=$(bin)PixelCabling.in -tag=$(tags) $(PixelCabling_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCabling_makefile) PixelCabling
else
$(cmt_local_PixelCabling_makefile) : $(PixelCabling_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCabling.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCabling) ] || \
	  [ ! -f $(cmt_final_setup_PixelCabling) ] || \
	  $(not_PixelCabling_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCabling_extratags) build constituent_makefile -out=$(cmt_local_PixelCabling_makefile) PixelCabling

PixelCabling :: $(PixelCabling_dependencies) $(cmt_local_PixelCabling_makefile) dirs PixelCablingdirs
	$(echo) "(constituents.make) Starting PixelCabling"
	@if test -f $(cmt_local_PixelCabling_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_makefile) PixelCabling; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCabling_makefile) PixelCabling
	$(echo) "(constituents.make) PixelCabling done"

clean :: PixelCablingclean ;

PixelCablingclean :: $(PixelCablingclean_dependencies) ##$(cmt_local_PixelCabling_makefile)
	$(echo) "(constituents.make) Starting PixelCablingclean"
	@-if test -f $(cmt_local_PixelCabling_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_makefile) PixelCablingclean; \
	fi
	$(echo) "(constituents.make) PixelCablingclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCabling_makefile) PixelCablingclean

##	  /bin/rm -f $(cmt_local_PixelCabling_makefile) $(bin)PixelCabling_dependencies.make

install :: PixelCablinginstall ;

PixelCablinginstall :: $(PixelCabling_dependencies) $(cmt_local_PixelCabling_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCabling_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCabling_makefile) install
...
$(foreach d,$(PixelCabling_dependencies),$(eval $(d)uninstall_dependencies += PixelCablinguninstall))

PixelCablinguninstall : $(PixelCablinguninstall_dependencies) ##$(cmt_local_PixelCabling_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCabling_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCabling_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCablinguninstall ;
...
PixelCablingConf_extratags = -tag_add=target_PixelCablingConf
...
not_PixelCablingConf_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCablingConf_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCablingConfdirs :
	@if test ! -d $(bin)PixelCablingConf; then $(mkdir) -p $(bin)PixelCablingConf; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCablingConf
else
PixelCablingConfdirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCablingConf_makefile) : $(PixelCablingConf_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingConf.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingConf_extratags) build constituent_config -out=$(cmt_local_PixelCablingConf_makefile) PixelCablingConf
else
$(cmt_local_PixelCablingConf_makefile) : $(PixelCablingConf_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingConf) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingConf) ] || \
	  $(not_PixelCablingConf_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCablingConf.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingConf_extratags) build constituent_config -out=$(cmt_local_PixelCablingConf_makefile) PixelCablingConf; \
	  fi
endif
...
$(cmt_local_PixelCablingConf_makefile) : $(PixelCablingConf_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingConf.make"; \
	  $(cmtexe) -f=$(bin)PixelCablingConf.in -tag=$(tags) $(PixelCablingConf_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCablingConf_makefile) PixelCablingConf
else
$(cmt_local_PixelCablingConf_makefile) : $(PixelCablingConf_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCablingConf.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingConf) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingConf) ] || \
	  $(not_PixelCablingConf_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCablingConf_extratags) build constituent_makefile -out=$(cmt_local_PixelCablingConf_makefile) PixelCablingConf

PixelCablingConf :: $(PixelCablingConf_dependencies) $(cmt_local_PixelCablingConf_makefile) dirs PixelCablingConfdirs
	$(echo) "(constituents.make) Starting PixelCablingConf"
	@if test -f $(cmt_local_PixelCablingConf_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingConf_makefile) PixelCablingConf; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingConf_makefile) PixelCablingConf
	$(echo) "(constituents.make) PixelCablingConf done"

clean :: PixelCablingConfclean ;

PixelCablingConfclean :: $(PixelCablingConfclean_dependencies) ##$(cmt_local_PixelCablingConf_makefile)
	$(echo) "(constituents.make) Starting PixelCablingConfclean"
	@-if test -f $(cmt_local_PixelCablingConf_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingConf_makefile) PixelCablingConfclean; \
	fi
	$(echo) "(constituents.make) PixelCablingConfclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCablingConf_makefile) PixelCablingConfclean

##	  /bin/rm -f $(cmt_local_PixelCablingConf_makefile) $(bin)PixelCablingConf_dependencies.make

install :: PixelCablingConfinstall ;

PixelCablingConfinstall :: $(PixelCablingConf_dependencies) $(cmt_local_PixelCablingConf_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCablingConf_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingConf_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCablingConf_makefile) install
	$(echo) "(constituents.make) $@ done"
...
$(foreach d,$(PixelCablingConf_dependencies),$(eval $(d)uninstall_dependencies += PixelCablingConfuninstall))

PixelCablingConfuninstall : $(PixelCablingConfuninstall_dependencies) ##$(cmt_local_PixelCablingConf_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCablingConf_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingConf_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingConf_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCablingConfuninstall ;
...
PixelCabling_python_init_extratags = -tag_add=target_PixelCabling_python_init
...
not_PixelCabling_python_init_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCabling_python_init_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCabling_python_initdirs :
	@if test ! -d $(bin)PixelCabling_python_init; then $(mkdir) -p $(bin)PixelCabling_python_init; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCabling_python_init
else
PixelCabling_python_initdirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCabling_python_init_makefile) : $(PixelCabling_python_init_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCabling_python_init.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCabling_python_init_extratags) build constituent_config -out=$(cmt_local_PixelCabling_python_init_makefile) PixelCabling_python_init
else
$(cmt_local_PixelCabling_python_init_makefile) : $(PixelCabling_python_init_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCabling_python_init) ] || \
	  [ ! -f $(cmt_final_setup_PixelCabling_python_init) ] || \
	  $(not_PixelCabling_python_init_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCabling_python_init.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCabling_python_init_extratags) build constituent_config -out=$(cmt_local_PixelCabling_python_init_makefile) PixelCabling_python_init; \
	  fi
endif
...
$(cmt_local_PixelCabling_python_init_makefile) : $(PixelCabling_python_init_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCabling_python_init.make"; \
	  $(cmtexe) -f=$(bin)PixelCabling_python_init.in -tag=$(tags) $(PixelCabling_python_init_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCabling_python_init_makefile) PixelCabling_python_init
else
$(cmt_local_PixelCabling_python_init_makefile) : $(PixelCabling_python_init_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCabling_python_init.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCabling_python_init) ] || \
	  [ ! -f $(cmt_final_setup_PixelCabling_python_init) ] || \
	  $(not_PixelCabling_python_init_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCabling_python_init_extratags) build constituent_makefile -out=$(cmt_local_PixelCabling_python_init_makefile) PixelCabling_python_init

PixelCabling_python_init :: $(PixelCabling_python_init_dependencies) $(cmt_local_PixelCabling_python_init_makefile) dirs PixelCabling_python_initdirs
	$(echo) "(constituents.make) Starting PixelCabling_python_init"
	@if test -f $(cmt_local_PixelCabling_python_init_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_python_init_makefile) PixelCabling_python_init; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCabling_python_init_makefile) PixelCabling_python_init
	$(echo) "(constituents.make) PixelCabling_python_init done"

clean :: PixelCabling_python_initclean ;

PixelCabling_python_initclean :: $(PixelCabling_python_initclean_dependencies) ##$(cmt_local_PixelCabling_python_init_makefile)
	$(echo) "(constituents.make) Starting PixelCabling_python_initclean"
	@-if test -f $(cmt_local_PixelCabling_python_init_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_python_init_makefile) PixelCabling_python_initclean; \
	fi
	$(echo) "(constituents.make) PixelCabling_python_initclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCabling_python_init_makefile) PixelCabling_python_initclean

##	  /bin/rm -f $(cmt_local_PixelCabling_python_init_makefile) $(bin)PixelCabling_python_init_dependencies.make

install :: PixelCabling_python_initinstall ;

PixelCabling_python_initinstall :: $(PixelCabling_python_init_dependencies) $(cmt_local_PixelCabling_python_init_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCabling_python_init_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_python_init_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCabling_python_init_makefile) install
	$(echo) "(constituents.make) $@ done"
...
$(foreach d,$(PixelCabling_python_init_dependencies),$(eval $(d)uninstall_dependencies += PixelCabling_python_inituninstall))

PixelCabling_python_inituninstall : $(PixelCabling_python_inituninstall_dependencies) ##$(cmt_local_PixelCabling_python_init_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCabling_python_init_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_python_init_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCabling_python_init_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCabling_python_inituninstall ;
...
cmt_PixelCablingConfDbMerge_has_no_target_tag = 1

#--------------------------------------

ifdef cmt_PixelCablingConfDbMerge_has_target_tag

cmt_local_tagfile_PixelCablingConfDbMerge = $(bin)$(PixelCabling_tag)_PixelCablingConfDbMerge.make
cmt_final_setup_PixelCablingConfDbMerge = $(bin)setup_PixelCablingConfDbMerge.make
cmt_local_PixelCablingConfDbMerge_makefile = $(bin)PixelCablingConfDbMerge.make

PixelCablingConfDbMerge_extratags = -tag_add=target_PixelCablingConfDbMerge

else

cmt_local_tagfile_PixelCablingConfDbMerge = $(bin)$(PixelCabling_tag).make
cmt_final_setup_PixelCablingConfDbMerge = $(bin)setup.make
cmt_local_PixelCablingConfDbMerge_makefile = $(bin)PixelCablingConfDbMerge.make

endif

not_PixelCablingConfDbMerge_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCablingConfDbMerge_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCablingConfDbMergedirs :
	@if test ! -d $(bin)PixelCablingConfDbMerge; then $(mkdir) -p $(bin)PixelCablingConfDbMerge; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCablingConfDbMerge
else
PixelCablingConfDbMergedirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCablingConfDbMerge_makefile) : $(PixelCablingConfDbMerge_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingConfDbMerge.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingConfDbMerge_extratags) build constituent_config -out=$(cmt_local_PixelCablingConfDbMerge_makefile) PixelCablingConfDbMerge
else
$(cmt_local_PixelCablingConfDbMerge_makefile) : $(PixelCablingConfDbMerge_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingConfDbMerge) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingConfDbMerge) ] || \
	  $(not_PixelCablingConfDbMerge_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCablingConfDbMerge.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingConfDbMerge_extratags) build constituent_config -out=$(cmt_local_PixelCablingConfDbMerge_makefile) PixelCablingConfDbMerge; \
	  fi
endif
...
$(cmt_local_PixelCablingConfDbMerge_makefile) : $(PixelCablingConfDbMerge_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingConfDbMerge.make"; \
	  $(cmtexe) -f=$(bin)PixelCablingConfDbMerge.in -tag=$(tags) $(PixelCablingConfDbMerge_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCablingConfDbMerge_makefile) PixelCablingConfDbMerge
else
$(cmt_local_PixelCablingConfDbMerge_makefile) : $(PixelCablingConfDbMerge_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCablingConfDbMerge.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingConfDbMerge) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingConfDbMerge) ] || \
	  $(not_PixelCablingConfDbMerge_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCablingConfDbMerge_extratags) build constituent_makefile -out=$(cmt_local_PixelCablingConfDbMerge_makefile) PixelCablingConfDbMerge

PixelCablingConfDbMerge :: $(PixelCablingConfDbMerge_dependencies) $(cmt_local_PixelCablingConfDbMerge_makefile) dirs PixelCablingConfDbMergedirs
	$(echo) "(constituents.make) Starting PixelCablingConfDbMerge"
	@if test -f $(cmt_local_PixelCablingConfDbMerge_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingConfDbMerge_makefile) PixelCablingConfDbMerge; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingConfDbMerge_makefile) PixelCablingConfDbMerge
	$(echo) "(constituents.make) PixelCablingConfDbMerge done"

clean :: PixelCablingConfDbMergeclean ;

PixelCablingConfDbMergeclean :: $(PixelCablingConfDbMergeclean_dependencies) ##$(cmt_local_PixelCablingConfDbMerge_makefile)
	$(echo) "(constituents.make) Starting PixelCablingConfDbMergeclean"
	@-if test -f $(cmt_local_PixelCablingConfDbMerge_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingConfDbMerge_makefile) PixelCablingConfDbMergeclean; \
	fi
	$(echo) "(constituents.make) PixelCablingConfDbMergeclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCablingConfDbMerge_makefile) PixelCablingConfDbMergeclean

##	  /bin/rm -f $(cmt_local_PixelCablingConfDbMerge_makefile) $(bin)PixelCablingConfDbMerge_dependencies.make

install :: PixelCablingConfDbMergeinstall ;

PixelCablingConfDbMergeinstall :: $(PixelCablingConfDbMerge_dependencies) $(cmt_local_PixelCablingConfDbMerge_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCablingConfDbMerge_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingConfDbMerge_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCablingConfDbMerge_makefile) install
	$(echo) "(constituents.make) $@ done"

uninstall : PixelCablingConfDbMergeuninstall

$(foreach d,$(PixelCablingConfDbMerge_dependencies),$(eval $(d)uninstall_dependencies += PixelCablingConfDbMergeuninstall))

PixelCablingConfDbMergeuninstall : $(PixelCablingConfDbMergeuninstall_dependencies) ##$(cmt_local_PixelCablingConfDbMerge_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCablingConfDbMerge_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingConfDbMerge_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingConfDbMerge_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCablingConfDbMergeuninstall ;
...
PixelCablingComponentsList_extratags = -tag_add=target_PixelCablingComponentsList
...
not_PixelCablingComponentsList_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCablingComponentsList_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCablingComponentsListdirs :
	@if test ! -d $(bin)PixelCablingComponentsList; then $(mkdir) -p $(bin)PixelCablingComponentsList; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCablingComponentsList
else
PixelCablingComponentsListdirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCablingComponentsList_makefile) : $(PixelCablingComponentsList_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingComponentsList.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingComponentsList_extratags) build constituent_config -out=$(cmt_local_PixelCablingComponentsList_makefile) PixelCablingComponentsList
else
$(cmt_local_PixelCablingComponentsList_makefile) : $(PixelCablingComponentsList_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingComponentsList) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingComponentsList) ] || \
	  $(not_PixelCablingComponentsList_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCablingComponentsList.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingComponentsList_extratags) build constituent_config -out=$(cmt_local_PixelCablingComponentsList_makefile) PixelCablingComponentsList; \
	  fi
endif
...
$(cmt_local_PixelCablingComponentsList_makefile) : $(PixelCablingComponentsList_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingComponentsList.make"; \
	  $(cmtexe) -f=$(bin)PixelCablingComponentsList.in -tag=$(tags) $(PixelCablingComponentsList_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCablingComponentsList_makefile) PixelCablingComponentsList
else
$(cmt_local_PixelCablingComponentsList_makefile) : $(PixelCablingComponentsList_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCablingComponentsList.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingComponentsList) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingComponentsList) ] || \
	  $(not_PixelCablingComponentsList_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCablingComponentsList_extratags) build constituent_makefile -out=$(cmt_local_PixelCablingComponentsList_makefile) PixelCablingComponentsList

PixelCablingComponentsList :: $(PixelCablingComponentsList_dependencies) $(cmt_local_PixelCablingComponentsList_makefile) dirs PixelCablingComponentsListdirs
	$(echo) "(constituents.make) Starting PixelCablingComponentsList"
	@if test -f $(cmt_local_PixelCablingComponentsList_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingComponentsList_makefile) PixelCablingComponentsList; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingComponentsList_makefile) PixelCablingComponentsList
	$(echo) "(constituents.make) PixelCablingComponentsList done"

clean :: PixelCablingComponentsListclean ;

PixelCablingComponentsListclean :: $(PixelCablingComponentsListclean_dependencies) ##$(cmt_local_PixelCablingComponentsList_makefile)
	$(echo) "(constituents.make) Starting PixelCablingComponentsListclean"
	@-if test -f $(cmt_local_PixelCablingComponentsList_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingComponentsList_makefile) PixelCablingComponentsListclean; \
	fi
	$(echo) "(constituents.make) PixelCablingComponentsListclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCablingComponentsList_makefile) PixelCablingComponentsListclean

##	  /bin/rm -f $(cmt_local_PixelCablingComponentsList_makefile) $(bin)PixelCablingComponentsList_dependencies.make

install :: PixelCablingComponentsListinstall ;

PixelCablingComponentsListinstall :: $(PixelCablingComponentsList_dependencies) $(cmt_local_PixelCablingComponentsList_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCablingComponentsList_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingComponentsList_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCablingComponentsList_makefile) install
	$(echo) "(constituents.make) $@ done"
...
$(foreach d,$(PixelCablingComponentsList_dependencies),$(eval $(d)uninstall_dependencies += PixelCablingComponentsListuninstall))

PixelCablingComponentsListuninstall : $(PixelCablingComponentsListuninstall_dependencies) ##$(cmt_local_PixelCablingComponentsList_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCablingComponentsList_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingComponentsList_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingComponentsList_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCablingComponentsListuninstall ;
...
PixelCablingMergeComponentsList_extratags = -tag_add=target_PixelCablingMergeComponentsList
...
not_PixelCablingMergeComponentsList_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCablingMergeComponentsList_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCablingMergeComponentsListdirs :
	@if test ! -d $(bin)PixelCablingMergeComponentsList; then $(mkdir) -p $(bin)PixelCablingMergeComponentsList; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCablingMergeComponentsList
else
PixelCablingMergeComponentsListdirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCablingMergeComponentsList_makefile) : $(PixelCablingMergeComponentsList_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingMergeComponentsList.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingMergeComponentsList_extratags) build constituent_config -out=$(cmt_local_PixelCablingMergeComponentsList_makefile) PixelCablingMergeComponentsList
else
$(cmt_local_PixelCablingMergeComponentsList_makefile) : $(PixelCablingMergeComponentsList_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingMergeComponentsList) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingMergeComponentsList) ] || \
	  $(not_PixelCablingMergeComponentsList_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCablingMergeComponentsList.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingMergeComponentsList_extratags) build constituent_config -out=$(cmt_local_PixelCablingMergeComponentsList_makefile) PixelCablingMergeComponentsList; \
	  fi
endif
...
$(cmt_local_PixelCablingMergeComponentsList_makefile) : $(PixelCablingMergeComponentsList_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingMergeComponentsList.make"; \
	  $(cmtexe) -f=$(bin)PixelCablingMergeComponentsList.in -tag=$(tags) $(PixelCablingMergeComponentsList_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCablingMergeComponentsList_makefile) PixelCablingMergeComponentsList
else
$(cmt_local_PixelCablingMergeComponentsList_makefile) : $(PixelCablingMergeComponentsList_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCablingMergeComponentsList.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingMergeComponentsList) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingMergeComponentsList) ] || \
	  $(not_PixelCablingMergeComponentsList_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCablingMergeComponentsList_extratags) build constituent_makefile -out=$(cmt_local_PixelCablingMergeComponentsList_makefile) PixelCablingMergeComponentsList

PixelCablingMergeComponentsList :: $(PixelCablingMergeComponentsList_dependencies) $(cmt_local_PixelCablingMergeComponentsList_makefile) dirs PixelCablingMergeComponentsListdirs
	$(echo) "(constituents.make) Starting PixelCablingMergeComponentsList"
	@if test -f $(cmt_local_PixelCablingMergeComponentsList_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingMergeComponentsList_makefile) PixelCablingMergeComponentsList; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingMergeComponentsList_makefile) PixelCablingMergeComponentsList
	$(echo) "(constituents.make) PixelCablingMergeComponentsList done"

clean :: PixelCablingMergeComponentsListclean ;

PixelCablingMergeComponentsListclean :: $(PixelCablingMergeComponentsListclean_dependencies) ##$(cmt_local_PixelCablingMergeComponentsList_makefile)
	$(echo) "(constituents.make) Starting PixelCablingMergeComponentsListclean"
	@-if test -f $(cmt_local_PixelCablingMergeComponentsList_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingMergeComponentsList_makefile) PixelCablingMergeComponentsListclean; \
	fi
	$(echo) "(constituents.make) PixelCablingMergeComponentsListclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCablingMergeComponentsList_makefile) PixelCablingMergeComponentsListclean

##	  /bin/rm -f $(cmt_local_PixelCablingMergeComponentsList_makefile) $(bin)PixelCablingMergeComponentsList_dependencies.make

install :: PixelCablingMergeComponentsListinstall ;

PixelCablingMergeComponentsListinstall :: $(PixelCablingMergeComponentsList_dependencies) $(cmt_local_PixelCablingMergeComponentsList_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCablingMergeComponentsList_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingMergeComponentsList_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCablingMergeComponentsList_makefile) install
	$(echo) "(constituents.make) $@ done"

uninstall : PixelCablingMergeComponentsListuninstall

$(foreach d,$(PixelCablingMergeComponentsList_dependencies),$(eval $(d)uninstall_dependencies += PixelCablingMergeComponentsListuninstall))

PixelCablingMergeComponentsListuninstall : $(PixelCablingMergeComponentsListuninstall_dependencies) ##$(cmt_local_PixelCablingMergeComponentsList_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCablingMergeComponentsList_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingMergeComponentsList_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingMergeComponentsList_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCablingMergeComponentsListuninstall ;
...
cmt_PixelCabling_optdebug_library_has_no_target_tag = 1

#--------------------------------------

ifdef cmt_PixelCabling_optdebug_library_has_target_tag

cmt_local_tagfile_PixelCabling_optdebug_library = $(bin)$(PixelCabling_tag)_PixelCabling_optdebug_library.make
cmt_final_setup_PixelCabling_optdebug_library = $(bin)setup_PixelCabling_optdebug_library.make
cmt_local_PixelCabling_optdebug_library_makefile = $(bin)PixelCabling_optdebug_library.make

PixelCabling_optdebug_library_extratags = -tag_add=target_PixelCabling_optdebug_library

else

cmt_local_tagfile_PixelCabling_optdebug_library = $(bin)$(PixelCabling_tag).make
cmt_final_setup_PixelCabling_optdebug_library = $(bin)setup.make
cmt_local_PixelCabling_optdebug_library_makefile = $(bin)PixelCabling_optdebug_library.make

endif

not_PixelCabling_optdebug_library_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCabling_optdebug_library_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCabling_optdebug_librarydirs :
	@if test ! -d $(bin)PixelCabling_optdebug_library; then $(mkdir) -p $(bin)PixelCabling_optdebug_library; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCabling_optdebug_library
else
PixelCabling_optdebug_librarydirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCabling_optdebug_library_makefile) : $(PixelCabling_optdebug_library_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCabling_optdebug_library.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCabling_optdebug_library_extratags) build constituent_config -out=$(cmt_local_PixelCabling_optdebug_library_makefile) PixelCabling_optdebug_library
else
$(cmt_local_PixelCabling_optdebug_library_makefile) : $(PixelCabling_optdebug_library_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCabling_optdebug_library) ] || \
	  [ ! -f $(cmt_final_setup_PixelCabling_optdebug_library) ] || \
	  $(not_PixelCabling_optdebug_library_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCabling_optdebug_library.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCabling_optdebug_library_extratags) build constituent_config -out=$(cmt_local_PixelCabling_optdebug_library_makefile) PixelCabling_optdebug_library; \
	  fi
endif
...
$(cmt_local_PixelCabling_optdebug_library_makefile) : $(PixelCabling_optdebug_library_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCabling_optdebug_library.make"; \
	  $(cmtexe) -f=$(bin)PixelCabling_optdebug_library.in -tag=$(tags) $(PixelCabling_optdebug_library_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCabling_optdebug_library_makefile) PixelCabling_optdebug_library
else
$(cmt_local_PixelCabling_optdebug_library_makefile) : $(PixelCabling_optdebug_library_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCabling_optdebug_library.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCabling_optdebug_library) ] || \
	  [ ! -f $(cmt_final_setup_PixelCabling_optdebug_library) ] || \
	  $(not_PixelCabling_optdebug_library_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCabling_optdebug_library_extratags) build constituent_makefile -out=$(cmt_local_PixelCabling_optdebug_library_makefile) PixelCabling_optdebug_library

PixelCabling_optdebug_library :: $(PixelCabling_optdebug_library_dependencies) $(cmt_local_PixelCabling_optdebug_library_makefile) dirs PixelCabling_optdebug_librarydirs
	$(echo) "(constituents.make) Starting PixelCabling_optdebug_library"
	@if test -f $(cmt_local_PixelCabling_optdebug_library_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_optdebug_library_makefile) PixelCabling_optdebug_library; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCabling_optdebug_library_makefile) PixelCabling_optdebug_library
	$(echo) "(constituents.make) PixelCabling_optdebug_library done"

clean :: PixelCabling_optdebug_libraryclean ;

PixelCabling_optdebug_libraryclean :: $(PixelCabling_optdebug_libraryclean_dependencies) ##$(cmt_local_PixelCabling_optdebug_library_makefile)
	$(echo) "(constituents.make) Starting PixelCabling_optdebug_libraryclean"
	@-if test -f $(cmt_local_PixelCabling_optdebug_library_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_optdebug_library_makefile) PixelCabling_optdebug_libraryclean; \
	fi
	$(echo) "(constituents.make) PixelCabling_optdebug_libraryclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCabling_optdebug_library_makefile) PixelCabling_optdebug_libraryclean

##	  /bin/rm -f $(cmt_local_PixelCabling_optdebug_library_makefile) $(bin)PixelCabling_optdebug_library_dependencies.make

install :: PixelCabling_optdebug_libraryinstall ;

PixelCabling_optdebug_libraryinstall :: $(PixelCabling_optdebug_library_dependencies) $(cmt_local_PixelCabling_optdebug_library_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCabling_optdebug_library_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_optdebug_library_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCabling_optdebug_library_makefile) install
	$(echo) "(constituents.make) $@ done"

uninstall : PixelCabling_optdebug_libraryuninstall

$(foreach d,$(PixelCabling_optdebug_library_dependencies),$(eval $(d)uninstall_dependencies += PixelCabling_optdebug_libraryuninstall))

PixelCabling_optdebug_libraryuninstall : $(PixelCabling_optdebug_libraryuninstall_dependencies) ##$(cmt_local_PixelCabling_optdebug_library_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCabling_optdebug_library_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_optdebug_library_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCabling_optdebug_library_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCabling_optdebug_libraryuninstall ;
...
PixelCablingCLIDDB_extratags = -tag_add=target_PixelCablingCLIDDB
...
not_PixelCablingCLIDDB_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCablingCLIDDB_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCablingCLIDDBdirs :
	@if test ! -d $(bin)PixelCablingCLIDDB; then $(mkdir) -p $(bin)PixelCablingCLIDDB; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCablingCLIDDB
else
PixelCablingCLIDDBdirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCablingCLIDDB_makefile) : $(PixelCablingCLIDDB_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingCLIDDB.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingCLIDDB_extratags) build constituent_config -out=$(cmt_local_PixelCablingCLIDDB_makefile) PixelCablingCLIDDB
else
$(cmt_local_PixelCablingCLIDDB_makefile) : $(PixelCablingCLIDDB_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingCLIDDB) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingCLIDDB) ] || \
	  $(not_PixelCablingCLIDDB_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCablingCLIDDB.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingCLIDDB_extratags) build constituent_config -out=$(cmt_local_PixelCablingCLIDDB_makefile) PixelCablingCLIDDB; \
	  fi
endif
...
$(cmt_local_PixelCablingCLIDDB_makefile) : $(PixelCablingCLIDDB_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingCLIDDB.make"; \
	  $(cmtexe) -f=$(bin)PixelCablingCLIDDB.in -tag=$(tags) $(PixelCablingCLIDDB_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCablingCLIDDB_makefile) PixelCablingCLIDDB
else
$(cmt_local_PixelCablingCLIDDB_makefile) : $(PixelCablingCLIDDB_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCablingCLIDDB.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingCLIDDB) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingCLIDDB) ] || \
	  $(not_PixelCablingCLIDDB_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCablingCLIDDB_extratags) build constituent_makefile -out=$(cmt_local_PixelCablingCLIDDB_makefile) PixelCablingCLIDDB

PixelCablingCLIDDB :: $(PixelCablingCLIDDB_dependencies) $(cmt_local_PixelCablingCLIDDB_makefile) dirs PixelCablingCLIDDBdirs
	$(echo) "(constituents.make) Starting PixelCablingCLIDDB"
	@if test -f $(cmt_local_PixelCablingCLIDDB_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingCLIDDB_makefile) PixelCablingCLIDDB; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingCLIDDB_makefile) PixelCablingCLIDDB
	$(echo) "(constituents.make) PixelCablingCLIDDB done"

clean :: PixelCablingCLIDDBclean ;

PixelCablingCLIDDBclean :: $(PixelCablingCLIDDBclean_dependencies) ##$(cmt_local_PixelCablingCLIDDB_makefile)
	$(echo) "(constituents.make) Starting PixelCablingCLIDDBclean"
	@-if test -f $(cmt_local_PixelCablingCLIDDB_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingCLIDDB_makefile) PixelCablingCLIDDBclean; \
	fi
	$(echo) "(constituents.make) PixelCablingCLIDDBclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCablingCLIDDB_makefile) PixelCablingCLIDDBclean

##	  /bin/rm -f $(cmt_local_PixelCablingCLIDDB_makefile) $(bin)PixelCablingCLIDDB_dependencies.make

install :: PixelCablingCLIDDBinstall ;

PixelCablingCLIDDBinstall :: $(PixelCablingCLIDDB_dependencies) $(cmt_local_PixelCablingCLIDDB_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCablingCLIDDB_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingCLIDDB_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCablingCLIDDB_makefile) install
	$(echo) "(constituents.make) $@ done"

uninstall : PixelCablingCLIDDBuninstall

$(foreach d,$(PixelCablingCLIDDB_dependencies),$(eval $(d)uninstall_dependencies += PixelCablingCLIDDBuninstall))

PixelCablingCLIDDBuninstall : $(PixelCablingCLIDDBuninstall_dependencies) ##$(cmt_local_PixelCablingCLIDDB_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCablingCLIDDB_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingCLIDDB_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingCLIDDB_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCablingCLIDDBuninstall ;
...
cmt_local_tagfile_install_python_modules = $(bin)$(PixelCabling_tag)_install_python_modules.make
...
cmt_local_tagfile_install_python_modules = $(bin)$(PixelCabling_tag).make
...
cmt_PixelCablingrchk_has_target_tag = 1

#--------------------------------------

ifdef cmt_PixelCablingrchk_has_target_tag

cmt_local_tagfile_PixelCablingrchk = $(bin)$(PixelCabling_tag)_PixelCablingrchk.make
cmt_final_setup_PixelCablingrchk = $(bin)setup_PixelCablingrchk.make
cmt_local_PixelCablingrchk_makefile = $(bin)PixelCablingrchk.make

PixelCablingrchk_extratags = -tag_add=target_PixelCablingrchk

else

cmt_local_tagfile_PixelCablingrchk = $(bin)$(PixelCabling_tag).make
cmt_final_setup_PixelCablingrchk = $(bin)setup.make
cmt_local_PixelCablingrchk_makefile = $(bin)PixelCablingrchk.make

endif

not_PixelCablingrchk_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCablingrchk_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCablingrchkdirs :
	@if test ! -d $(bin)PixelCablingrchk; then $(mkdir) -p $(bin)PixelCablingrchk; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCablingrchk
else
PixelCablingrchkdirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCablingrchk_makefile) : $(PixelCablingrchk_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingrchk.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingrchk_extratags) build constituent_config -out=$(cmt_local_PixelCablingrchk_makefile) PixelCablingrchk
else
$(cmt_local_PixelCablingrchk_makefile) : $(PixelCablingrchk_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingrchk) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingrchk) ] || \
	  $(not_PixelCablingrchk_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCablingrchk.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCablingrchk_extratags) build constituent_config -out=$(cmt_local_PixelCablingrchk_makefile) PixelCablingrchk; \
	  fi
endif
...
$(cmt_local_PixelCablingrchk_makefile) : $(PixelCablingrchk_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCablingrchk.make"; \
	  $(cmtexe) -f=$(bin)PixelCablingrchk.in -tag=$(tags) $(PixelCablingrchk_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCablingrchk_makefile) PixelCablingrchk
else
$(cmt_local_PixelCablingrchk_makefile) : $(PixelCablingrchk_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCablingrchk.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCablingrchk) ] || \
	  [ ! -f $(cmt_final_setup_PixelCablingrchk) ] || \
	  $(not_PixelCablingrchk_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCablingrchk_extratags) build constituent_makefile -out=$(cmt_local_PixelCablingrchk_makefile) PixelCablingrchk

PixelCablingrchk :: $(PixelCablingrchk_dependencies) $(cmt_local_PixelCablingrchk_makefile) dirs PixelCablingrchkdirs
	$(echo) "(constituents.make) Starting PixelCablingrchk"
	@if test -f $(cmt_local_PixelCablingrchk_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingrchk_makefile) PixelCablingrchk; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingrchk_makefile) PixelCablingrchk
	$(echo) "(constituents.make) PixelCablingrchk done"

clean :: PixelCablingrchkclean ;

PixelCablingrchkclean :: $(PixelCablingrchkclean_dependencies) ##$(cmt_local_PixelCablingrchk_makefile)
	$(echo) "(constituents.make) Starting PixelCablingrchkclean"
	@-if test -f $(cmt_local_PixelCablingrchk_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingrchk_makefile) PixelCablingrchkclean; \
	fi
	$(echo) "(constituents.make) PixelCablingrchkclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCablingrchk_makefile) PixelCablingrchkclean

##	  /bin/rm -f $(cmt_local_PixelCablingrchk_makefile) $(bin)PixelCablingrchk_dependencies.make

install :: PixelCablingrchkinstall ;

PixelCablingrchkinstall :: $(PixelCablingrchk_dependencies) $(cmt_local_PixelCablingrchk_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCablingrchk_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingrchk_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCablingrchk_makefile) install
	$(echo) "(constituents.make) $@ done"

uninstall : PixelCablingrchkuninstall

$(foreach d,$(PixelCablingrchk_dependencies),$(eval $(d)uninstall_dependencies += PixelCablingrchkuninstall))

PixelCablingrchkuninstall : $(PixelCablingrchkuninstall_dependencies) ##$(cmt_local_PixelCablingrchk_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCablingrchk_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCablingrchk_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCablingrchk_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCablingrchkuninstall ;
...
cmt_local_tagfile_install_root_include_path = $(bin)$(PixelCabling_tag)_install_root_include_path.make
...
cmt_local_tagfile_install_root_include_path = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_install_includes = $(bin)$(PixelCabling_tag)_install_includes.make
...
cmt_local_tagfile_install_includes = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_make = $(bin)$(PixelCabling_tag)_make.make
...
cmt_local_tagfile_make = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_CompilePython = $(bin)$(PixelCabling_tag)_CompilePython.make
...
cmt_local_tagfile_CompilePython = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_qmtest_run = $(bin)$(PixelCabling_tag)_qmtest_run.make
...
cmt_local_tagfile_qmtest_run = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_qmtest_summarize = $(bin)$(PixelCabling_tag)_qmtest_summarize.make
...
cmt_local_tagfile_qmtest_summarize = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_TestPackage = $(bin)$(PixelCabling_tag)_TestPackage.make
...
cmt_local_tagfile_TestPackage = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_TestProject = $(bin)$(PixelCabling_tag)_TestProject.make
...
cmt_local_tagfile_TestProject = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_new_rootsys = $(bin)$(PixelCabling_tag)_new_rootsys.make
...
cmt_local_tagfile_new_rootsys = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_doxygen = $(bin)$(PixelCabling_tag)_doxygen.make
...
cmt_local_tagfile_doxygen = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_post_install = $(bin)$(PixelCabling_tag)_post_install.make
...
cmt_local_tagfile_post_install = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_post_merge_rootmap = $(bin)$(PixelCabling_tag)_post_merge_rootmap.make
...
cmt_local_tagfile_post_merge_rootmap = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_post_merge_genconfdb = $(bin)$(PixelCabling_tag)_post_merge_genconfdb.make
...
cmt_local_tagfile_post_merge_genconfdb = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_post_build_tpcnvdb = $(bin)$(PixelCabling_tag)_post_build_tpcnvdb.make
...
cmt_local_tagfile_post_build_tpcnvdb = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_all_post_constituents = $(bin)$(PixelCabling_tag)_all_post_constituents.make
...
cmt_local_tagfile_all_post_constituents = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_checkreq = $(bin)$(PixelCabling_tag)_checkreq.make
...
cmt_local_tagfile_checkreq = $(bin)$(PixelCabling_tag).make
...
cmt_PixelCabling_NICOS_Fix_debuginfo_has_no_target_tag = 1

#--------------------------------------

ifdef cmt_PixelCabling_NICOS_Fix_debuginfo_has_target_tag

cmt_local_tagfile_PixelCabling_NICOS_Fix_debuginfo = $(bin)$(PixelCabling_tag)_PixelCabling_NICOS_Fix_debuginfo.make
cmt_final_setup_PixelCabling_NICOS_Fix_debuginfo = $(bin)setup_PixelCabling_NICOS_Fix_debuginfo.make
cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile = $(bin)PixelCabling_NICOS_Fix_debuginfo.make

PixelCabling_NICOS_Fix_debuginfo_extratags = -tag_add=target_PixelCabling_NICOS_Fix_debuginfo

else

cmt_local_tagfile_PixelCabling_NICOS_Fix_debuginfo = $(bin)$(PixelCabling_tag).make
cmt_final_setup_PixelCabling_NICOS_Fix_debuginfo = $(bin)setup.make
cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile = $(bin)PixelCabling_NICOS_Fix_debuginfo.make

endif

not_PixelCabling_NICOS_Fix_debuginfo_dependencies = { n=0; for p in $?; do m=0; for d in $(PixelCabling_NICOS_Fix_debuginfo_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
PixelCabling_NICOS_Fix_debuginfodirs :
	@if test ! -d $(bin)PixelCabling_NICOS_Fix_debuginfo; then $(mkdir) -p $(bin)PixelCabling_NICOS_Fix_debuginfo; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)PixelCabling_NICOS_Fix_debuginfo
else
PixelCabling_NICOS_Fix_debuginfodirs : ;
endif
...
ifndef QUICK
$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) : $(PixelCabling_NICOS_Fix_debuginfo_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCabling_NICOS_Fix_debuginfo.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCabling_NICOS_Fix_debuginfo_extratags) build constituent_config -out=$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) PixelCabling_NICOS_Fix_debuginfo
else
$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) : $(PixelCabling_NICOS_Fix_debuginfo_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCabling_NICOS_Fix_debuginfo) ] || \
	  [ ! -f $(cmt_final_setup_PixelCabling_NICOS_Fix_debuginfo) ] || \
	  $(not_PixelCabling_NICOS_Fix_debuginfo_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building PixelCabling_NICOS_Fix_debuginfo.make"; \
	  $(cmtexe) -tag=$(tags) $(PixelCabling_NICOS_Fix_debuginfo_extratags) build constituent_config -out=$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) PixelCabling_NICOS_Fix_debuginfo; \
	  fi
endif
...
$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) : $(PixelCabling_NICOS_Fix_debuginfo_dependencies) build_library_links
	$(echo) "(constituents.make) Building PixelCabling_NICOS_Fix_debuginfo.make"; \
	  $(cmtexe) -f=$(bin)PixelCabling_NICOS_Fix_debuginfo.in -tag=$(tags) $(PixelCabling_NICOS_Fix_debuginfo_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) PixelCabling_NICOS_Fix_debuginfo
else
$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) : $(PixelCabling_NICOS_Fix_debuginfo_dependencies) $(cmt_build_library_linksstamp) $(bin)PixelCabling_NICOS_Fix_debuginfo.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_PixelCabling_NICOS_Fix_debuginfo) ] || \
	  [ ! -f $(cmt_final_setup_PixelCabling_NICOS_Fix_debuginfo) ] || \
	  $(not_PixelCabling_NICOS_Fix_debuginfo_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(PixelCabling_NICOS_Fix_debuginfo_extratags) build constituent_makefile -out=$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) PixelCabling_NICOS_Fix_debuginfo

PixelCabling_NICOS_Fix_debuginfo :: $(PixelCabling_NICOS_Fix_debuginfo_dependencies) $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) dirs PixelCabling_NICOS_Fix_debuginfodirs
	$(echo) "(constituents.make) Starting PixelCabling_NICOS_Fix_debuginfo"
	@if test -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) PixelCabling_NICOS_Fix_debuginfo; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) PixelCabling_NICOS_Fix_debuginfo
	$(echo) "(constituents.make) PixelCabling_NICOS_Fix_debuginfo done"

clean :: PixelCabling_NICOS_Fix_debuginfoclean ;

PixelCabling_NICOS_Fix_debuginfoclean :: $(PixelCabling_NICOS_Fix_debuginfoclean_dependencies) ##$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile)
	$(echo) "(constituents.make) Starting PixelCabling_NICOS_Fix_debuginfoclean"
	@-if test -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) PixelCabling_NICOS_Fix_debuginfoclean; \
	fi
	$(echo) "(constituents.make) PixelCabling_NICOS_Fix_debuginfoclean done"
#	@-$(MAKE) -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) PixelCabling_NICOS_Fix_debuginfoclean

##	  /bin/rm -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) $(bin)PixelCabling_NICOS_Fix_debuginfo_dependencies.make

install :: PixelCabling_NICOS_Fix_debuginfoinstall ;

PixelCabling_NICOS_Fix_debuginfoinstall :: $(PixelCabling_NICOS_Fix_debuginfo_dependencies) $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) install; \
	  fi
#	@-$(MAKE) -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) install
	$(echo) "(constituents.make) $@ done"

uninstall : PixelCabling_NICOS_Fix_debuginfouninstall

$(foreach d,$(PixelCabling_NICOS_Fix_debuginfo_dependencies),$(eval $(d)uninstall_dependencies += PixelCabling_NICOS_Fix_debuginfouninstall))

PixelCabling_NICOS_Fix_debuginfouninstall : $(PixelCabling_NICOS_Fix_debuginfouninstall_dependencies) ##$(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile); then \
	  $(MAKE) -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_PixelCabling_NICOS_Fix_debuginfo_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: PixelCabling_NICOS_Fix_debuginfouninstall ;
...
cmt_local_tagfile_check_install_runtime = $(bin)$(PixelCabling_tag)_check_install_runtime.make
...
cmt_local_tagfile_check_install_runtime = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_check_install_joboptions = $(bin)$(PixelCabling_tag)_check_install_joboptions.make
...
cmt_local_tagfile_check_install_joboptions = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_check_install_python_modules = $(bin)$(PixelCabling_tag)_check_install_python_modules.make
...
cmt_local_tagfile_check_install_python_modules = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_find_packages = $(bin)$(PixelCabling_tag)_find_packages.make
...
cmt_local_tagfile_find_packages = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_compile = $(bin)$(PixelCabling_tag)_compile.make
...
cmt_local_tagfile_compile = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_compile_pkg = $(bin)$(PixelCabling_tag)_compile_pkg.make
...
cmt_local_tagfile_compile_pkg = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_new_analysisapp = $(bin)$(PixelCabling_tag)_new_analysisapp.make
...
cmt_local_tagfile_new_analysisapp = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_new_pkg = $(bin)$(PixelCabling_tag)_new_pkg.make
...
cmt_local_tagfile_new_pkg = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_new_alg = $(bin)$(PixelCabling_tag)_new_alg.make
...
cmt_local_tagfile_new_alg = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_new_analysisalg = $(bin)$(PixelCabling_tag)_new_analysisalg.make
...
cmt_local_tagfile_new_analysisalg = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_new_asgtool = $(bin)$(PixelCabling_tag)_new_asgtool.make
...
cmt_local_tagfile_new_asgtool = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_new_jobo = $(bin)$(PixelCabling_tag)_new_jobo.make
...
cmt_local_tagfile_new_jobo = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_new_skeleton = $(bin)$(PixelCabling_tag)_new_skeleton.make
...
cmt_local_tagfile_new_skeleton = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_make_skeleton = $(bin)$(PixelCabling_tag)_make_skeleton.make
...
cmt_local_tagfile_make_skeleton = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_checkout_pkg = $(bin)$(PixelCabling_tag)_checkout_pkg.make
...
cmt_local_tagfile_checkout_pkg = $(bin)$(PixelCabling_tag).make
...
cmt_local_tagfile_cppcheck = $(bin)$(PixelCabling_tag)_cppcheck.make
...
cmt_local_tagfile_cppcheck = $(bin)$(PixelCabling_tag).make
