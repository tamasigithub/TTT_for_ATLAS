...
void SiDigitizationSurfaceChargeInserter::operator ()
    ...
    else { // Probably CMOS which cannot fit in strip part alone
        const InDetDD::StripBoxDesign *cmosDesign = dynamic_cast<const InDetDD::StripBoxDesign *> (&(m_sielement->design()));
        if (cmosDesign) {
            int row, strip;
            cmosDesign->closestStripRowOfPosition(scharge.position(), strip, row);
            InDetDD::SiCellId diode(row, strip);
            m_chargedDiodes->add(diode, scharge.charge());
        }
        else {
            std::cout << "Invalid diode, and was not cmosDesign\n";
        }
    }
}
} // anonymous namespace
